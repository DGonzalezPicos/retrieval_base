...
kwargs = {'lw': 0.7, 'color': 'forestgreen'}
...
for i in range(n_orders):
    ...
    ax[0].plot(wave[i], m_flux_full[i], lw=lw, color='forestgreen')
    res_i = d_flux[i] - m_flux_full[i]
    # relative residual amplitude
    rra = np.nanmean(np.abs(res_i) / d_flux[i])
    
    ax[1].plot(wave[i], res_i, lw=lw, color='forestgreen')
    text = 'RRA = ' if i == 0 else ''
    text += f'{rra*100:.2f}%'
    
    x_text = 0.02
    if not save_every_order:
        x_text += i*0.12
    ax[1].text(x_text, 0.8, text, transform=ax[1].transAxes, color='k')
    ...
    if save_every_order:
        ...
        ax[1].set(xlabel='Wavelength / nm', ylabel='Residuals', xlim=(np.nanmin(wave[i]), np.nanmax(wave[i])))
        fig_name_i = str(fig_name).replace('.png', f'_order_{i}.png')
        fig.savefig(fig_name_i, dpi=300, bbox_inches='tight', facecolor='none')
        print(f' --> Saved {fig_name_i}')
        # clear the axes
        ax[0].cla()
        ax[1].cla()
