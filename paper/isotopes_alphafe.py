...
# calculate alpha enhacement from [C/H] and [Fe/H] for GJ 745 AB
# C_H_A = np.log10(1.04e-4)
# Fe_H_
def alpha_fe(alpha_H, Fe_H, alpha_fe_solar=(8.46-7.46), Fe_H_solar=(7.46-12.0)):
    """
    Calculate alpha enhancement from [C/H] and [Fe/H].
    
    Parameters:
    ===========
    alpha_H: float
        Logarithmic abundance of the alpha element relative to hydrogen.
    Fe_H: float
        Logarithmic abundance of iron relative to hydrogen.
    alpha_fe_solar: tuple, optional
        Logarithmic abundance of the alpha element and iron relative to hydrogen in the sun.
    Fe_H_solar: tuple, optional
        Logarithmic abundance of iron relative to hydrogen in the sun.
        
    Returns:
    =======
    alpha_fe: float
        Alpha enhancement, i.e. the difference between the logarithmic abundance of the
        alpha element and iron relative to hydrogen, normalized to the solar value.
    """
    return alpha_H - Fe_H - alpha_fe_solar - Fe_H_solar
alpha_fe_A = alpha_fe(np.log10(1.04e-4), crossfield_dict['carbon']['Gl 745 A'][-1][0])
alpha_fe_B = alpha_fe(np.log10(0.995e-4), crossfield_dict['carbon']['Gl 745 B'][-1][0])
alpha_fe_err = 0.05 # from metallicity measurement propagation
# update dictionary
if x_param == '[alpha/Fe]':
    for s in ['carbon', 'oxygen']:
        crossfield_dict[s]['Gl 745 A'][-1] = (alpha_fe_A, alpha_fe_err)
        crossfield_dict[s]['Gl 745 B'][-1] = (alpha_fe_B, alpha_fe_err)

print(crossfield_dict)
...
for i, isotope in enumerate(isotopes):
    ...
    if i == 1:
        xlabel = x_param if x_param != '[alpha/Fe]' else r'[$\alpha$/Fe]'
        ...
        b = 0.068
        cbar_ax = fig.add_axes([0.798, b, 0.030, top-b])
...
fig_name = nat_path + f'{main_label}_isotopes_{x_param_label}_{metallicity_ref}{loglog_label}{table_id_label}{sub10pc_label}_2sigma.pdf'
