...
def main(target, ax, order=0, offset=0.0, run=None, text_x=None, offset_x=0.0, **kwargs):
    ...
    if show_name:
        text_pos = [np.nanmin(wave[order, mask[order]]), 
                    np.nanquantile(flux[order, :len(flux[order]//2)], 0.90)-0.14]
                    # np.nanquantile(flux[order, :len(flux[order]//2)], 0.90)-0.04]
        if text_x is not None:
            text_pos[0] = text_x[0]
        if kwargs.get('text_y', None) is not None:
            print(f' Setting text_y to {kwargs.get("text_y")}')
...
def plot(orders, name, text_x=None, xlim=None, show_lines=False, **kwargs):
    ...
    fig = plt.figure(figsize=(width_mm*mm_to_inch, height_mm*mm_to_inch))  # Increase the height to accommodate spacing
    gs = gridspec.GridSpec(11, 10, wspace=0.2, hspace=0.00, 
                            top=0.95, bottom=0.12, left=0.12, right=0.89)  # Set spacing
    fig.text(0.36, 0.03, r'Wavelength [nm]', ha='center', va='center', fontsize=font_size)
    fig.text(0.01, 0.5, r'Normalized flux', ha='center', va='center', rotation='vertical', fontsize=font_size)
    
    ax_label_y = 0.01
    ax_labels = dict(a=(0.126, 0.80), b=(0.126, 0.725), 
                     c=(0.126, 0.50), d=(0.126, 0.422),
                     e=(0.126, 0.195), f=(0.126, 0.118),
                     g=(0.595, 0.61), h=(0.595, 0.225),
                     )
    for k, v in ax_labels.items():
        fig.text(v[0], v[1] + ax_label_y, k, fontsize=font_size, fontweight='bold', zorder=0, alpha=0.90)
    ...
    for i in range(3):
        ...
        xticks_i = xticks[i]
        xticks_i_labels = [f'{t:.0f}' for t in xticks_i]
       
        raxes[-1].set_xticks(xticks_i)
        raxes[-1].set_xticklabels(xticks_i_labels)
    ...
    ccf_ax.set_title('Cross-correlation function', fontsize=font_size)
    ccf_ax.set_ylabel('S/N', fontsize=font_size)
    ccf_ax.set_ylim(-9.99, None)
    
    # Ensure tick labels follow journal requirements
    for axis in [ccf_ax.xaxis, ccf_ax.yaxis, ccf_res_ax.xaxis, ccf_res_ax.yaxis]:
        axis.set_tick_params(labelsize=font_size)
    
    # Update axis labels
    ccf_res_ax.set_xlabel(r'RV [km s$^{-1}$]', fontsize=font_size)
    ccf_res_ax.set_ylabel('CCF - ACF', fontsize=font_size)
    for key in ['h', 'v']:
    ...
    fig_name = nat_path + 'nat_fig_2_revised.pdf'
...
if __name__ == '__main__':
    ...
    plot(orders, name=name, text_x=text_x, xlim=xlim_list, 
        #  text_y=1.07, 
        text_y=1.18, # 2025-08-02
         show_lines=True)