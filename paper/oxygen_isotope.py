...
def main(target, x, xerr=None, label='', ax=None, run=None, xytext=None,**kwargs):
    ...
    oxygen_isotope_posterior_file = base_path + target + '/retrieval_outputs/' + run + f'/{main_label}_oxygen_isotope_posterior.npy'
    if not os.path.exists(oxygen_isotope_posterior_file):
        ...
        log_oxygen_ratio_id = param_keys.index(key)

        # make scatter plot with one point corresponding to Teff vs log(12CO/13CO)
        # take uncertainties from posterior quantiles 

        oxygen_isotope_posterior = 10.0**posterior[:, log_oxygen_ratio_id]
        np.save(oxygen_isotope_posterior_file, oxygen_isotope_posterior)
        print(f' {target}: Saved oxygen isotope posterior to {oxygen_isotope_posterior_file}')
    else:
        oxygen_isotope_posterior = np.load(oxygen_isotope_posterior_file)
        
    q=[0.16, 0.5, 0.84]
    oxygen_isotope_quantiles = np.quantile(oxygen_isotope_posterior, q)
        

    ax_new = ax is None
    ax = ax or plt.gca()
    print(f' {target}: log {main_label} O16/O18 = {oxygen_isotope_quantiles[1]:.2f} +{oxygen_isotope_quantiles[2]-oxygen_isotope_quantiles[1]:.2f} -{oxygen_isotope_quantiles[1]-oxygen_isotope_quantiles[0]:.2f}\n')
    ...
    y = oxygen_isotope_quantiles[1]
    lolims=False
    fmt = 'o'
    if target=='gl699' and plot_lower_limit_gl699:
        y -= oxygen_isotope_quantiles[0]
        lolims = oxygen_isotope_quantiles[2]-oxygen_isotope_quantiles[1]
        fmt = 's'
    ax.errorbar(x, y, 
                xerr=xerr,
                yerr=[[oxygen_isotope_quantiles[1]-oxygen_isotope_quantiles[0]], [oxygen_isotope_quantiles[2]-oxygen_isotope_quantiles[1]]],
                fmt=fmt, 
        ...
        # ax.annotate('', xy=(x, oxygen_isotope_quantiles[1]-oxygen_isotope_quantiles[0]), 
        #             xytext=(x, (oxygen_isotope_quantiles[1]-oxygen_isotope_quantiles[0]) - 0.1),
        #             arrowprops=dict(facecolor='black', shrink=0.05, width=1.5, headwidth=8)
        # )
    # if x<xlim[1]:
    if xytext is not None:
        # add text with target name next to point, offset text from point
        ax.annotate(label.replace('gl', 'Gl '), (x, oxygen_isotope_quantiles[1]), textcoords="offset points", xytext=xytext, ha='left',
                    fontsize=8, color=kwargs.get('color', 'k'), alpha=0.9)
        
    

    if ax_new:
        ax.set_ylabel(r'$^{16}$O/$^{18}$O')
        
    return oxygen_isotope_quantiles
...
crossfield = {'Gl 745 A': [(1220, 260), (3454, 31), (-0.43, 0.05)],
...
sun = (529.7, 1.7) # solar wind McKeegan et al. 2011
# sun = {'Teff (K)': ()
...
ism = [557, 30] # ISM value from Wilson et al. 1999
...
ax.set_ylabel(r'$^{16}$O/$^{18}$O' + f' ({main_label})')
# plt.show()
...
fig_name = base_path + f'paper/latex/figures/{main_label}_oxygen_isotope_{x_param_label}{loglog_label}.pdf'
