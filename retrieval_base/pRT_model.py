...
class pRT_model:
    ...
    def get_model_spectrum(self, get_contr=False, get_full_spectrum=False, fast=False):
        ...
        for i, atm_i in enumerate(self.atm):
            ...
            d_pc = self.params.get('d_pc', 1e3/self.params['parallax'])
            flux_i *= (
            ...
            if len(self.disk_species) > 0 and (self.params['gratings'][i]=='g395h'):
                ...
                n_slabs = self.disk_kwargs.get('n_slabs', 0)
                ...
                for ds_i in self.disk_species:
                    ...
                    if ds_i == '13CO':
                        ...
                        N_mol_12CO = self.params.get(f'N_mol_12CO', self.params[f'N_mol'])
                        
                        N_mol = N_mol_12CO / carbon_isotope_ratio
                        T_ex = self.params.get(f'T_ex_12CO', self.params[f'T_ex'])
                    else:
                        N_mol = self.params.get(f'N_mol_{ds_i}', self.params[f'N_mol'])
                        T_ex = self.params.get(f'T_ex_{ds_i}', self.params[f'T_ex'])
