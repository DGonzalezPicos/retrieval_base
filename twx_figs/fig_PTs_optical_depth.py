...
for t, target in enumerate(runs.keys()):
    ...
    # p, t, cf, logg = get_PT(path, target, runs[target], cache=False)
    ret = get_PT(path, target, runs[target], cache=False)
    p = ret.PT.pressure
    t = ret.PT.temperature_envelopes[3,:]
    total_tau = ret.pRT_atm['NIRSpec'].atm[0].total_tau[0] # shape (n_wave, n_species, n_layers)
    # tau = np.nanmean(total_tau, axis=(0,1)) # shape (n_layers)
    tau = total_tau[0,0,:]
    plt.plot(t, tau, color=colors[target]['model'], alpha=0.4, lw=1.0)
    plt.ylim(0, 3)
    plt.show()
    # assert False
    # p, t, cf, logg = ret.PT.pressure, ret.PT.temperature_envelopes, ret.PT.int_contr_em['NIRSpec'], np.median(ret.logg_posterior)
    assert False
    print(t)
...
# plt.show()
fig_name = path_figures / f'fig_PTs_optical_depth.pdf'
